...
use crocodicstudio\crudbooster\import\DefaultImport;
use CRUDBooster;
...
class CBController extends Controller
{
    ...
    public function validation($id = null)
    {
        ...
        $array_input = [];
        //dd($request_all);
        foreach ($this->data_inputan as $key => $di) {
            $ai = [];
            $name = $di['name'];
            //estaba verificando porque se salta la validacion de un valor nulo en textarea

            if($request_all[$name] == null){
                $request_all[$name] = "";
            }
...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function getImportData()
    {
        ...
        if (request('file') && ! request('import')) {
            ...
            $rows = Excel::toArray(new DefaultImport, $file);
            //$rows =  (new DefaultImport)->toArray($file);
            //dd($rows);
            $data_import_column = [];
